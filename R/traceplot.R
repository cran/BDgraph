...
#~     y            <- sapply( allGraphs, function(x) length( which( unlist( strsplit( as.character(x), "" ) ) == 1 ) ) )
	...
	{
		plot( x = 1 : length(allGraphs), sizeAllGraphs, type = "l", main = main, cex.main = 1.5, cex.lab = 1.3, cex.axis = 1.2,
	...
	if ( acf == TRUE & pacf == TRUE )
	{
		...
		plot( x = 1 : length(allGraphs), sizeAllGraphs, type = "l", main = main,
			   ylab = "Graph size", xlab = "Iteration", ... )
		abline( h = lin, col = "red" )	  
