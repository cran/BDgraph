...
#~ 			data = data / sd( data[ , 1] )
	...
	if( ( is.null( multi.update ) ) && ( p > 10 & iter > p ) )
	{
	...
	if( save.all == TRUE )
	{
		if( ( method == "ggm" ) && ( algorithm == "bdmcmc" ) && is.null( multi_update ) )
		{
		...
		if( ( method == "gcgm" ) && ( algorithm == "bdmcmc" ) && is.null( multi_update ) )
		{
...
#~ 	system( 'clear' )  # To clear the console in R
