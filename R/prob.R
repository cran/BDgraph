...
prob = function( output, g = 4, G = NULL )
{
	...
	if (is.null(G))
	{
		...
		for ( i in 1 : g )
		{
			gi <- sampleGraphs[ which( graphWeights == sort( graphWeights, decreasing = TRUE )[i] ) ]
			gv <- 0 * gv
		...
		return( list( best.G = graphi, prob.G = sort( graphWeights, decreasing = TRUE )[1 : g] / sum( graphWeights ) ) )
