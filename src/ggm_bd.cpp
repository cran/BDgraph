...
////////////////////////////////////////////////////////////////////////////////
// bdmcmc algoirthm for GGMs with exact value of normalizing constant for D = I_p
////////////////////////////////////////////////////////////////////////////////
			 ...
			 int *b, int *bstar, double Ds[], double *threshold )
{
	...
	double sumWeights = 0.0, rate, maxRates, sumRates, sumDiag, K022, a11, b1 = *b, sigmaj11, threshold_C = *threshold;
...
// STEP 1: calculating birth-death rates
	...
	// Finished (i,j) = 1-----------------------------------------------------------|
				...
				nustar = b1;
				for( k = 0; k < dim; k++ ) nustar += G[i * dim + k] * G[j * dim + k];
...
// Finished saving result -----------------------------------------------------|	
	...
	*sizeSampleG = sizeSampleGraph;
	...
	for( i = 0; i < pxp; i++ ) Khat[i] /= sumWeights;		
}
        
////////////////////////////////////////////////////////////////////////////////
// bdmcmc algoirthm with exact value of normalizing constant for D = I_p
////////////////////////////////////////////////////////////////////////////////
void bdmcmcExactPhat( int *iter, int *burnin, int G[], double Ts[], double K[], int *p, 
			 double Khat[], double phat[],
			 int *b, int *bstar, double Ds[], double *threshold )
{
	int iteration = *iter, burn_in = *burnin;

	int selectedEdgei, selectedEdgej, selectedEdgeij;
	int row, col, rowCol, i, j, k, ij, jj, Dsjj, Dsij, counter, nustar, one = 1, two = 2;
	int dim = *p, pxp = dim * dim, p1 = dim - 1, p1xp1 = p1 * p1, p2 = dim - 2, p2xp2 = p2 * p2, p2x2 = p2 * 2;
