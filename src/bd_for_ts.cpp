...
*/
void bdmcmc_for_multi_dim( int *iter, int *burnin, int G[], int g_space[], double Ls[], double r_K[], double i_K[], int *p, 
...
{
	int iteration = *iter, burn_in = *burnin, T = *freq;
	int index_selected_edge, selected_edge_i, selected_edge_j, selected_edge_ij;
	int ip, i, j, t, k, counter = 0, dim = *p, pxp = dim * dim;
	...
	for( int i_mcmc = 0; i_mcmc < iteration; i_mcmc++ )
	{
		...
		{
			txpxp   = t*pxp;	
			rates_cbdmcmc_parallel( &rates[0], G, &index_row[0], &index_col[0], &sub_qp, &r_Ds[txpxp], &i_Ds[txpxp], &r_sigma[txpxp], &i_sigma[txpxp], &r_K[txpxp], &i_K[txpxp], &b[t], &dim );
